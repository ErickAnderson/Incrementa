...
* **Modular Structure:**
    * `src/core/` (`game.ts`, `base-entity.ts`, `timer.ts`, `unlock-manager.ts`, `event-emitter.ts`, etc.)
    * `src/entities/`
        * `resources/` (`resource.ts`)
        * `buildings/` (`building.ts`, `producer-building.ts`, `miner.ts`, `factory.ts`, `storage.ts`)
        * `upgrades/` (`upgrade.ts`)
    * `src/utils/` (Utility functions, e.g., `id-generator.ts`)
    * `src/types/` (Shared type definitions, e.g., `cost-definition.ts`, `production-io.ts`)
...
graph TD
  ...
  resources --> gold.resource.ts

  buildings --> iron-mine.building.ts

  upgrades --> faster-mining.upgrade.ts
...
src/
  ...
  entities/
    resources/
      gold.resource.ts // Example concrete resource
    buildings/
      iron-mine.building.ts // Example concrete building
    upgrades/
      faster-mining.upgrade.ts // Example concrete upgrade
  utils/
  types/
    index.ts
    common.types.ts
...
import { IronMine } from "incrementa/entities/buildings/iron-mine"; // If exposing pre-fabs
```
...
* **UI Bindings via Event System:** All `BaseEntity` instances (and their derivatives) include a simple event emitter (`on`, `emit`). Developers can subscribe to events like `amountChanged` (for Resources), `buildComplete` (for Buildings), `unlocked`, `levelChanged`, etc., to update any frontend framework.
